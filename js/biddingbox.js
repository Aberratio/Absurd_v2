...
function start() {
    var div_content = "";
        for(i = 0; i <= 34; i++) {
            div_content = div_content + '<div class="bid" onclick="declare('+i+')" id="bid'+i+'">' + declaration[i] + '</div>';
            if((i + 1) % 5 == 0) {
    ...
    if(point_string.length > 0) {
        var div_content = "<table style='border-collapse: separate; border-spacing: 50px 0; margin-bottom: 50px;'><tr style='color: rgb(0, 255, 106);'><td>Kontrakt</td><td>Punkty</td></tr>";
        var res = point_string.split(";");
        
        for(var i = 0; i < res.length; i++) {
            res2 = res[i].split("=");
            div_content = div_content + '<tr><td>' + declaration[res2[0].trim()] + '</td><td>' + res2[1] + '</td></tr>';
        }
...
function declare(bid_index) {
    if(document.getElementById("turn").innerText == document.getElementById("hand").innerText) {
...
function write_bid(bid_index) {
    if((bid_index >= 0) && (bid_index <=38)) {
        if((bid_index >= 35) || (document.getElementById("bid" + bid_index).style.getPropertyValue('background') != "rgb(49, 49, 49)")) {
            if(bid_index <= 34) {
